...
class(df) # DF
summary(df) #summary
table(df$items)   #frequency distribution of items column
table(df$store)   #frequency distribution of items store
#Slice Data Frame: 
...
#It is also possible to select multiple columns
df[, c(1,4)]
...
df
subset(df, subset = price > 5)         # Select price above 5
subset(df, subset = store == 'TRUE')   # Select store == 'TRUE'