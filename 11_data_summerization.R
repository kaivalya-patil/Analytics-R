...
#mean of mpg & hp wrt(with respect to) gear types
...
addmargins(A=t1, margin=2) #default is sum; sum of each row, shown as column
...
#two function on rows/ col margins(sum of columns & mean of rows)
addmargins(A=t1, margin=c(1,2), FUN=c(sum,mean))
#two function on rows/ col margins(mean of columns & sum of rows)
addmargins(A=t1, margin=c(1,2), FUN=c(mean,sum))
#different functions on both rows and columns at same time
